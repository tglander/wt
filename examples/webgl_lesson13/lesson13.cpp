...
const std::string fragShad( // frag 1 ..  per vertex sahader.
"#ifdef GL_ES \n"
  ...
  : WApplication(env)
{
  ...
  root()->addWidget(new WText("Wt/WebGL Lesson 13: per-fragment lighting and multiple programs"));
  ...
  chBox = new WCheckBox("Use Per Fragment Lighting",root());
  chBox->setChecked();
  chBox->clicked().connect(this,&WebGLDemo::eventChecked);
  chBox2 = new WCheckBox("Use Textures",root());
  chBox2->setChecked();
  ...
  lightColB = new WLineEdit("0.0",root());
  ...
  root()->addWidget(new WBreak());
  root()->addWidget(new WText("R : "));
  ambLightColR = new WLineEdit("0.0",root());
  root()->addWidget(new WText(" G : "));
...
void WebGLDemo::updateShaders()
{
    ...
    drawGLWidget_->setShaders(vertShad,fragShad,vertShad2,fragShad2);
