  ...
  : WApplication(env)
{
  ...
  root()->addWidget(new WText("Wt/WebGL Lesson 14: specular highlights and loading a model"));
  ...
  root()->addWidget(new WBreak());
  root()->addWidget(new WText("Shiness : "));
  shiness = new WLineEdit("32.0",root());
  ...
  lightDirY = new WLineEdit("4.0",root());
  ...
  specColR = new WLineEdit("0.8",root());
  root()->addWidget(new WText(" G : "));
  specColG = new WLineEdit("0.8",root());
  root()->addWidget(new WText(" B : "));
  specColB = new WLineEdit("0.8",root());
  ...
  shiness->keyWentUp().connect(this,&WebGLDemo::eventKeyWentUp);
...
void WebGLDemo::eventKeyWentUp(const WKeyEvent& event){
    ...
    drawGLWidget_->updateLights(ambientColor,lightDirection,lightColor,specularColor,getVal(shiness->text().toUTF8()));
    drawGLWidget_->repaintGL(DrawGLWidget::PAINT_GL);
