...
void DrawGLWidget::initializeGL()
{
    ...
    lightDirection.assign(-10.0,4.0,-20.0);
    //lightDirection.normalize();
    //lightDirection.scale(-1.0);
    lightColor.assign(0.8);
    specularColor.assign(0.8);
...
void DrawGLWidget::initShaders()
{
    ...
    showSpecHighLightsUniform_  = getUniformLocation(shaderProgram_, "uShowSpecularHighlights");
    ...
    materialShininessUniform_  = getUniformLocation(shaderProgram_, "uMaterialShininess");
...
void DrawGLWidget::paintGL()
{
    ...
    mvMatrix.rotate(23.4,1.0,0.0,-1.0);
    ...
    uniform1i(useTexturesUniform_,useTextures);
    uniform1i(showSpecHighLightsUniform_,useSpecMap);
    ...
    uniform3f(pointLightingDiffuseColorUniform_,lightColor.values[0],lightColor.values[1],lightColor.values[2]);
    uniform1f(materialShininessUniform_,shiness);



    bindBuffer(ARRAY_BUFFER, Teapot_.VPBuffer_);
    vertexAttribPointer(vertexPositionAttribute_,
        Teapot_.VPBitemsize_,     // size: Every vertex has an X, Y and Z component
    ...
    bindBuffer(ARRAY_BUFFER, Teapot_.VNBuffer_);
    vertexAttribPointer(vertexNormalAttribute_,
        Teapot_.VNBitemsize_,     // size: Every vertex has an X, Y and Z component
        FLOAT, // type: They are floats
        false, // normalized: Please, do NOT normalize the vertices
        0, // stride: The first byte of the next vertex is located this
        //         amount of bytes further. The format of the VBO is
        ...
        //         Float32, hence 4 bytes large
        0);    // offset: The byte position of the first vertex in the buffer
    bindBuffer(ARRAY_BUFFER, Teapot_.TCBuffer_);
    vertexAttribPointer(textureCoordAttribute_,
       Teapot_.TCBitemsize_,     // size: Every vertex has RGB components
        FLOAT, // type: They are floats
        false, // normalized: Please, do NOT normalize the vertices
        0, // stride: The first byte of the next vertex is located this
        //         amount of bytes further. The format of the VBO is
        ...
        //         Float32, hence 4 bytes large
        0);    // offset: The byte position of the first vertex in the buffer
    bindBuffer(ELEMENT_ARRAY_BUFFER, Teapot_.VIBuffer_);
    drawElements(TRIANGLES,Teapot_.VIBnumel_,UNSIGNED_SHORT,0);
}


void DrawGLWidget::updateBools(bool useLights,bool useTex, bool useSpec){
...
void DrawGLWidget::updateLights(macVec3 newAmbientColor,macVec3 newLightDirection,macVec3 newLightColor,macVec3 newSpec,double newShiness)
{
...
void DrawGLWidget::createTeapot(){

    Buffer teapotVertexPositionBuffer;
    Buffer teapotVertexIndexBuffer;
    Buffer teapotTextureCoordBuffer;
    Buffer teapotVertexNormalBuffer;


    VicomItemMatrix<double> teapotVertices(3);
    VicomItemMatrix<double> teapotTexCoord(2);
    VicomItemMatrix<double> teapotNormals(3);
    VicomItemMatrix<unsigned short int> teapotIndices(3);
