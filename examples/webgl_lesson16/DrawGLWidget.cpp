...
void DrawGLWidget::initializeGL()
{
	...
	rotMatrix_screen = createJavaScriptMatrix4();
    //setClientSideWalkHandler(rotMatrix,0.005,0.005);
    setClientSideLookAtHandler(rotMatrix_screen,0,0,0,0,1,0,0.007,0.007);
...
void DrawGLWidget::initFrameBuffer(){
	rrtFramebuffer = createFramebuffer();
	bindFramebuffer(FRAMEBUFFER,rrtFramebuffer);
	int width = 512;
	int height = 512;

	rrtTexture = createTexture();
	bindTexture(TEXTURE_2D,rrtTexture);
	texParameteri(TEXTURE_2D,TEXTURE_MAG_FILTER,LINEAR);
	texParameteri(TEXTURE_2D,TEXTURE_MIN_FILTER,LINEAR_MIPMAP_NEAREST);
	generateMipmap(TEXTURE_2D);
	// it's missing in the Wt WGLWidget...
	// TODO
	texImage2D(TEXTURE_2D, 0, RGBA, width, height, 0, RGBA);// this function adds internally -> UNSIGNED_BYTE, "null"); 
	
	rrtRenderbuffer = createRenderbuffer();
	bindRenderbuffer(RENDERBUFFER,rrtRenderbuffer);
	...
	framebufferTexture2D(FRAMEBUFFER,COLOR_ATTACHMENT0, TEXTURE_2D, rrtTexture, 0);
	framebufferRenderbuffer(FRAMEBUFFER,DEPTH_ATTACHMENT,RENDERBUFFER,rrtRenderbuffer);

	bindTexture(TEXTURE_2D,printNull);
	bindFramebuffer(FRAMEBUFFER, printNull);
	bindRenderbuffer(RENDERBUFFER,printNull);
...
void DrawGLWidget::paintGL()
{
	...
	//////////////////////////
	// Draw what's in the framebuffer.
	bindFramebuffer(FRAMEBUFFER, rrtFramebuffer);
    ...
    if (!usePerFrag_){
        ...
        mvMatrix.translate(0.0,0.0,-2.0+zDepth);
	...
	bindFramebuffer(FRAMEBUFFER, printNull);
	////////////////////////////////////////////////////////////
	// Finished drawing in the framebuffer
	...
	if (!usePerFrag_){
        ...
        //set model-view matrix
		mvMatrix.setToIdentity();
		mvMatrix.translate(0.0,0.0,zDepth-1.0);
		identMatrix.setToIdentity();
        identMatrix.rotate(45,1.0,1.0,1.0);
		
		
        uniformMatrix4(cMatrixUniform_, (rotMatrix_screen*identMatrix));
        //set model-view matrix
        uniformMatrix4(mvMatrixUniform_, mvMatrix);
        //
        uniformMatrix4(nMatrixUniform_, (rotMatrix_screen*identMatrix).inverted().transposed());
        ...
        uniform1i(useLightingUniform_,useLighting);
        uniform1i(useTexturesUniform_,useTextures_);
        uniform3f(ambientColorUniform_,2.0,2.0,2.0);
        uniform3f(lightingDirectionUniform_,lightDirection.values[0],lightDirection.values[1],lightDirection.values[2]);
        uniform3f(directionalColorUniform_,2.0*lightColor.values[0],2.0*lightColor.values[1],2.0*lightColor.values[2]);
        ...
        uniformMatrix4(cMatrixUniform2_, (rotMatrix_screen*identMatrix));
        //set model-view matrix
        uniformMatrix4(mvMatrixUniform2_, mvMatrix);
        //
        uniformMatrix4(nMatrixUniform2_, (rotMatrix_screen*identMatrix).inverted().transposed());
        ...
        uniform1i(useLightingUniform2_,useLighting);
        uniform1i(useTexturesUniform2_,useTextures_);
        uniform3f(ambientColorUniform2_,2.0,2.0,2.0);
        uniform3f(lightingDirectionUniform2_,lightDirection.values[0],lightDirection.values[1],lightDirection.values[2]);
        uniform3f(directionalColorUniform2_,2.0*lightColor.values[0],2.0*lightColor.values[1],2.0*lightColor.values[2]);
...
void DrawGLWidget::setJavaScript(){
	...
	doJavaScript("function WTVicomGLAnim(){if (myTickValue=-"+sss.str()+"){}"+"if (!window.requestAnimationFrame) {"+
                                 "window.requestAnimationFrame = (function() {"+
                                  "return window.requestAnimationFrame ||"+
                                 "		 window.webkitRequestAnimationFrame ||"+
                                 "		 window.mozRequestAnimationFrame ||"+
                                 ...
                                 rotMatrix.jsRef()+",0.01,[0.0,1.0,0.0],"+rotMatrix.jsRef()+");"+                                 "  obj.paintGL();"
                                 "  window.requestAnimationFrame(WTVicomGLAnim);"+
                                 "}/**Vicomtech_Client_Side_Code*/; WTVicomGLAnim();");

	//clientSideFcn_screen.setJavaScript("function WTVicomGLAnim(){if (myTickValue=-"+sss.str()+"){}"+"if (!window.requestAnimationFrame) {"+
 //                                "window.requestAnimationFrame = (function() {"+
 //                                 "return window.requestAnimationFrame ||"+
 //                                "		 window.webkitRequestAnimationFrame ||"+
 //                                "		 window.mozRequestAnimationFrame ||"+
 ...
 //                                "return o ? o : {ctx: null};"+
 //                                "})()" +"; var ctx=obj.ctx; "+WT_CLASS ".glMatrix.mat4.rotate(" +
 //                                rotMatrix_screen.jsRef()+",0.005,[1.0,1.0,1.0],"+rotMatrix_screen.jsRef()+");"+                                 "  obj.paintGL();"
 //                                "  window.requestAnimationFrame(WTVicomGLAnim);"+
 //                                "}/**Vicomtech_Client_Side_Code*/; WTVicomGLAnim();");

	doJavaScript("function WTVicomGLAnim2(){if (myTickValue=-"+sss.str()+"){}"+"if (!window.requestAnimationFrame) {"+
                                 "window.requestAnimationFrame = (function() {"+
                                  "return window.requestAnimationFrame ||"+
                                 "		 window.webkitRequestAnimationFrame ||"+
                                 "		 window.mozRequestAnimationFrame ||"+
                                 ...
                                 "})()" +"; var ctx=obj.ctx; "+WT_CLASS ".glMatrix.mat4.rotate(" +
                                 rotMatrix_screen.jsRef()+",0.005,[1.0,1.0,1.0],"+rotMatrix_screen.jsRef()+");"+                                 "  obj.paintGL();"
                                 "  window.requestAnimationFrame(WTVicomGLAnim2);"+
                                 "}/**Vicomtech_Client_Side_Code*/; WTVicomGLAnim2();");
}
